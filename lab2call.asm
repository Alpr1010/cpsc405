...
_lab2call:     file format elf32-i386
...
int main(void) {
    ...
    1011:	e8 fc 02 00 00       	call   1312 <getcount>
    1016:	83 ec 04             	sub    $0x4,%esp
    1019:	50                   	push   %eax
    101a:	68 f0 16 00 00       	push   $0x16f0
    101f:	6a 01                	push   $0x1
    1021:	e8 aa 03 00 00       	call   13d0 <printf>
    
    exit();
    1026:	e8 47 02 00 00       	call   1272 <exit>
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    136f:	f7 f6                	div    %esi
    1371:	0f b6 92 14 17 00 00 	movzbl 0x1714(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    14e8:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
    14eb:	b8 0d 17 00 00       	mov    $0x170d,%eax
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1571:	a1 ac 19 00 00       	mov    0x19ac,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    15c9:	a3 ac 19 00 00       	mov    %eax,0x19ac
}
    15ce:	5b                   	pop    %ebx
    15cf:	5e                   	pop    %esi
    15d0:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    15ed:	a3 ac 19 00 00       	mov    %eax,0x19ac
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    160c:	8b 15 ac 19 00 00    	mov    0x19ac,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    1661:	39 05 ac 19 00 00    	cmp    %eax,0x19ac
  ...
  return freep;
    168b:	8b 15 ac 19 00 00    	mov    0x19ac,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16ac:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
    16af:	89 15 ac 19 00 00    	mov    %edx,0x19ac
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    16c6:	c7 05 ac 19 00 00 b0 	movl   $0x19b0,0x19ac
    16cd:	19 00 00 
    16d0:	c7 05 b0 19 00 00 b0 	movl   $0x19b0,0x19b0
    16d7:	19 00 00 
    base.s.size = 0;
    16da:	b8 b0 19 00 00       	mov    $0x19b0,%eax
    16df:	c7 05 b4 19 00 00 00 	movl   $0x0,0x19b4
    16e6:	00 00 00 
