...
printint(int fd, int xx, int base, int sgn)
{
 ...
 36f:	f7 f6                	div    %esi
 371:	0f b6 92 14 07 00 00 	movzbl 0x714(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 4e8:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 4eb:	b8 0d 07 00 00       	mov    $0x70d,%eax
...
free(void *ap)
{
 ...
 5c7:	89 08                	mov    %ecx,(%eax)
  freep = p;
 5c9:	a3 ac 09 00 00       	mov    %eax,0x9ac
}
 5ce:	5b                   	pop    %ebx
 5cf:	5e                   	pop    %esi
 5d0:	5f                   	pop    %edi
 ...
 5ea:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 5ed:	a3 ac 09 00 00       	mov    %eax,0x9ac
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 6cd:	09 00 00 
 6d0:	c7 05 b0 09 00 00 b0 	movl   $0x9b0,0x9b0
 6d7:	09 00 00 
    base.s.size = 0;
 6da:	b8 b0 09 00 00       	mov    $0x9b0,%eax
