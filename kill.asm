...
main(int argc, char **argv)
{
  ...
  if(argc < 2){
    ...
    104a:	50                   	push   %eax
    104b:	50                   	push   %eax
    104c:	68 20 17 00 00       	push   $0x1720
    1051:	6a 02                	push   $0x2
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    139f:	f7 f6                	div    %esi
    13a1:	0f b6 92 3c 17 00 00 	movzbl 0x173c(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    1518:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
    151b:	b8 34 17 00 00       	mov    $0x1734,%eax
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15a1:	a1 e0 19 00 00       	mov    0x19e0,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    15f9:	a3 e0 19 00 00       	mov    %eax,0x19e0
}
    15fe:	5b                   	pop    %ebx
    15ff:	5e                   	pop    %esi
    1600:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    161d:	a3 e0 19 00 00       	mov    %eax,0x19e0
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    163c:	8b 15 e0 19 00 00    	mov    0x19e0,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    1691:	39 05 e0 19 00 00    	cmp    %eax,0x19e0
  ...
  return freep;
    16bb:	8b 15 e0 19 00 00    	mov    0x19e0,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16dc:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
    16df:	89 15 e0 19 00 00    	mov    %edx,0x19e0
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    16f6:	c7 05 e0 19 00 00 e4 	movl   $0x19e4,0x19e0
    16fd:	19 00 00 
    1700:	c7 05 e4 19 00 00 e4 	movl   $0x19e4,0x19e4
    1707:	19 00 00 
    base.s.size = 0;
    170a:	b8 e4 19 00 00       	mov    $0x19e4,%eax
    170f:	c7 05 e8 19 00 00 00 	movl   $0x0,0x19e8
    1716:	00 00 00 
